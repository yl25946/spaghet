...
void Searcher::bench()
{
    const std::array<std::string, 50> Fens{// fens from alexandria, ultimately from bitgenie
    ...
    // time in seconds
    const double time = Fens.size() / 10;

    std::cout << "info string " << time << " seconds" << std::endl;
    std::cout << total_nodes << " nodes " << (uint64_t)(total_nodes / time) << " nps" << std::endl;
}