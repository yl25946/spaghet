...
void SEE_suite()
{
    ...
    {
        // std::string see_test = "2r5/1P4pk/p2p1b1p/5b1n/BB3p2/2R2p2/P1P2P2/4RK2 w - - | c3c8 | 500 | R";

        const auto test_data = SplitString(see_test, '|');
        Board board(test_data[0] + "0 1");

        Move move = parse_move(RemoveWhitespace(test_data[1]), board);

        // std::cout << "move flag: " << static_cast<int>(move.move_flag()) << '\n';

        const int threshold = std::stoi(RemoveWhitespace(test_data[2]));
        // int threshold = 300;
