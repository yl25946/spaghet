...
void MovePicker::score(const Board &board, SearchStack *ss, TranspositionTable &transposition_table, QuietHistory &history, CaptureHistory capthist, ContinuationHistory &conthist, Killers &killers, int threshold)
{
    ...
    for (int i = 0; i < move_list.size(); ++i)
    {
        ...
        if (!move_list.moves[i].is_quiet())
        {
            ...
            {
                // just hardcoded
                move_list.moves[i].score += 50 * piece_value[PIECES::WHITE_PAWN] - piece_value[PIECES::WHITE_PAWN] + (SEE(board, move_list.moves[i], threshold) ? CAPTURE_BONUS : -CAPTURE_BONUS);
                continue;
            }
            ...
            move_list.moves[i].score += 50 * (captured_piece_value + promotion_piece_value) + attacking_piece_value + (SEE(board, move_list.moves[i], threshold) ? CAPTURE_BONUS : -CAPTURE_BONUS);
